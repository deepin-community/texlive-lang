...
\hypersetup{%
  ...
  pdfauthor={Uwe Ziegenhagen, ww.uweziegenhagen.1de},%
  pdfsubject={TeX Live },%
...
\item [-repository {\textsl{url/path}}] zur Auswahl einer Installationsquelle, für weitere Erläuterung siehe den nächsten Kapitel.
...
Falls Sie ein lokales Repository verwenden (also entweder direkt einen Verzeichnispfad oder eine \texttt{file:/}-URL angeben), dann werden bevorzugt Archivdateien in einem Unterverzeichnis \dirname{archive} des Repository verwendet, selbst wenn ausgepackte Dateien vorhanden sind.
...
Web2C besteht aus einer Reihe von Programmen, die zusammen ein komplettes \TeX-System darstellen. Dazu gehören natürlich \TeX, \MF, \MP, {\BibTeX} usw.
...
Sprache \texttt{WEB} entwickelte Programme als Basis und übersetzt diese in C-Quell-Code. Darüber hinaus bietet das System viele Makros und Funktionen zur Nutzung der originalen \TeX-Software. Hier eine Liste der Basisprogramme eines \TeX-Systems:
...
\section{\TL 2023}\label{sec:2023news}


Windows: wie vorab angekündigt enthält \TL{} jetzt 64-bit Windows Binärdateien anstelle von 32-bit Dateien. Das neue Verzeichnis für die Binaries lautet 
...
\begin{itemize}
\item Vermeide Abstürze, die durch \cs{output} and \cs{write} ausgelöst wurden.
\item Kleinere Inner-Schleifen Optimierung.
\end{itemize}

\noindent Lua\TeX: 

\begin{itemize}
\item Prüfungen auf Lua-Level prüfen Datei-Zugriffe jetzt genau wie \TeX.
\item Wenn der Lua-Wert \code{texconfig.trace\_extra\_newline} gesetzt ist, dann werden Leerzeilen im Log zwischen den Makro-Parametern ausgegeben. Von 2020--2023 war dies der Standard.
\end{itemize}

\noindent MetaPost: \begin{itemize}\raggedright
\item Die MPFI Bibliothek für Intervall-Arithmetik wird jetzt mitgeliefert; mit \texttt{-{}-numbersystem=interval} wird sie aktiviert.
\end{itemize}

\noindent pdf\TeX: \begin{itemize}
\item Verlässlichere Textextrahierung auf BigEndian Systemen.
\item Mehr Checks für den Schutz vor kaputten pfb, fmt, pdf Dateien.
\item Verhindere das Schreiben von korrupten PDFs durch unübliche PDF-Einbettungen.
\item Handbuch wurde nach \LaTeX\ konvertiert und gründlich überarbeitet.
\end{itemize}

\noindent ep\TeX{}, eup\TeX{}: \begin{itemize}
\item \texttt{ptekf} als neues Werkzeug zur einfachen Konvertierung von Encodings.
\item \texttt{eptex} wird nicht mehr separat erzeugt; es läuft jetzt im \texttt{euptex} Legacy-Encoding Modus.
\end{itemize}

\noindent Xe\TeX{}: \begin{itemize}
\item Unicode Zeichen im Dateinamen, die über BMP hinausgehen, werden unterstützt.
\end{itemize}
 
\noindent Dvipdfmx: \begin{itemize}
\item Berechne den Zeitzonenversatz genau wie pdf\TeX.
\item Überarbeitete Variation-Selektoren, um mit dem neuen Adobe-Japan1-UCS2 CMap arbeiten zu können.
\item Fallback-Multibyte Zeichen in virtuellen Fonts für (u)p\TeX.
\end{itemize}

\noindent Kpathsea: \begin{itemize}
\item Unterstützt einen erweiterten Check für sichere Dateinamen  unter  \code{TEXMF}[\code{SYS}]\code{VAR}, für Lua\AllTeX{}; neue Funktionen und korrespondierende \code{kpsewhich} Optionen.
\item Unterstützt eine neue Variable \code{TEXMF\_OUTPUT\_DIRECTORY} (neben dem 
traditionellen \code{TEXMFOUTPUT}), sodasd Unterprogramme auf \texttt{-{}-output-directory} zugreifen können.
\end{itemize}

\noindent CWEB: \begin{itemize}
\item Unterstützt Lua\TeX{} als \TeX{} Engine.
\item Fixt einen Buffer Overflow in Zeilen ohne Breakpoint.
\end{itemize}

\noindent \code{tlmgr}: \begin{itemize}
\item Neue Aktion \code{bug} um Informationen zum Bug Reporting auszugeben.
\item Erlaubt die Nutzung eines eigenen  Root Zertifikats auf allen Plattformen mit dem Zweck, LWP zu propagieren, insbesondere unter MacOS.
\item Stelle fest, ob ein Remote Repository älter als die lokale Installation ist und verhindere ein Update aus der veralteten Quelle
\item Unterstützt das Setzen der Papiergröße unter \ConTeXt{}.
\item Stutze die Backups von tlpdb files für gespiegelte Repositories (main und tlcontrib).
\end{itemize}
