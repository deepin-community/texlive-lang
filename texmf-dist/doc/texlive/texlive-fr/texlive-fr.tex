...
%
% 2024/02/28: synced with 2024 English version, by Denis Bitouzé
...
\documentclass[german, english, french, 12pt]{article}
...
\email{texhax@tug.org} par mail. Mais, avant de poser votre question, nous vous
...
\item[TEXMFSYSVAR] -- c'est là que les scripts \cmdname{texconfig-sys},
  \cmdname{updmap-sys}, \cmdname{fmtutil-sys} et \cmdname{tlmgr} rangent les
...
\item[Installateur réseau, .zip ou .tar.gz] -- télécharger l'archive depuis le
  CTAN dans \dirname{systems/texlive/tlnet} ; l'URL
...
décompression de l'un de ces deux fichiers, les scripts \filename{install-tl} et
\filename{install-tl-windows.bat} se trouveront dans le sous-répertoire
\dirname{install-tl}.
...
que vous n'ayez mis en miroir le répertoire \dirname{tlnet} localement.
...
d'installation par défaut est \dirname{/usr/local/texlive/2024} sous Unix et
...
créer |C:\texlive\2024| (ou, plus généralement, |%SystemDrive%\texlive\2024|).
...
différentes variables d'environnement\footnote{Vraisemblablement
  \envname{TEXLIVE\_INSTALL\_PREFIX} ou \envname{TEXLIVE\_INSTALL\_TEXDIR}.}
...
section~\ref{sec:context} page~\pageref{sec:context}) ; par défaut il
...
\item[Pour \XeTeX :] sous :
  ...
  \item[systèmes Unix autres que \macOS{} :] la procédure est la suivante :
    ...
    \begin{enumerate*}
    \item copiez dans (généralement) \path+~/.fonts.conf.d/09-texlive.conf+ le
      fichier \filename{texlive-fontconfig.conf}, où \filename{~} représente
...
\label{sec:context} % conservation des deux labels pour assurer la compatibilité
                    % ascendante
...
\code{ls-R} ; à la place, \ConTeXt{} maintient une base de données de noms de
fichiers par utilisateur pour tous les arbres connus. L'exécution de
\code{mktexlsr} ne mettra pas à jour ces bases de données directement, mais
\ConTeXt{} les reconstruira automatiquement lors de sa prochaine exécution. (Les
fichiers spécifiques à \TL{} \code{cont-sys.mkiv} et \code{cont-sys.mkxl}
implémentent cela.)
...
\end{sverbatim}
Les fichiers résultants sont stockés sous \code{TEXMFCACHE}, dont la valeur par
...
\item[Prise en charge PostScript] -- pour les fichiers PostScript, un type de
  ...
  une entrée \cmdname{bitmap2eps} est ajoutée à l'entrée « Ouvrir avec... » du
...
À la place, un fichier \file{\dots/2024/texmf.cnf} est créé à la suite d'une
...
\file{\dots/2024/texmf-dist/web2c/texmf.cnf}. Vous ne devriez jamais éditer ce
...
\item Considérée comme une chaîne de caractères, \var{valeur} peut contenir
  ...
  Unix, afin de disposer d'un seul \file{texmf.cnf} qui puisse prendre en charge
...
\item[\texttt{--format=\var{name}}]\mbox{}\\
  ...
  fichiers de configuration \cmdname{dvips}, vous devez spécifier le nom connu
...
configuration générique (\file{config.ps}) et la liste des fontes \PS{}
\file{psfonts.map}.  Depuis l'édition 2004, les fichiers \file{.map} et les
fichiers de codage ont changé de place dans l'arborescence
\dirname{texmf}. Comme le suffixe \file{.ps} est ambigu, nous devons spécifier
...
\item Hironori Kitagawa, pour la maintenance de (e)p\TeX{} et la prise en charge
  afférente.
...
\item Takuji Tanaka, pour la maintenance de (e)(u)p\TeX{} et la prise en charge
  afférente.
...
\item Hironobu Yamashita, pour son travail sur p\TeX\ et la prise en charge
  associée.
...
\item Norbert Preining (\pkgname{aarch64-linux}, \pkgname{armhf-linux},
  \pkgname{x86\_64-linuxmusl}, \pkgname{i386-freebsd}, \pkgname{amd64-freebsd},
  \pkgname{i386-linux}, \pkgname{x86\_64-linux}, \pkgname{i386-solaris},
...
\item Boris Veytsman \& Vladimir Lomov (russe) ;
...
configuration idéale, car il prenait en charge déjà plusieurs plateformes et
avait été construit en gardant à l'esprit la portabilité entre systèmes.  Thomas
...
\item La prise en charge pour Alpha OSF a été supprimée (celui pour HPUX l'avait
  été auparavant), car personne disposant des machines nécessaires ne s'est
...
\item Un nouveau moteur combinant les fonctionnalités de \eTeX\ et d'\OMEGA,
  appelé Aleph, est disponible. Quelques informations se trouvent dans
  \OnCD{texmf-dist/doc/aleph/base} et
  à \url{https://texfaq.org/FAQ-enginedev}. Le format \LaTeX{} utilisant Aleph
s'appelle \pkgname{lamed}.
...
avec son homonyme japonais ; la prise en charge pour le polonais est assurée
...
à chaque exécution. Ceci est relié à la prise en charge de \MP{} en tant que
bibliothèque, qui est un autre changement important bien que peu visible par les
utilisateurs.
...
\begin{description}
\item[pdf\TeX :] nouveau paramètre \cs{pdfsuppresswarningpagegroup}
  d'avertissement de suppression ; nouvelles primitives pour des espaces
...
\item[\pkgname{langcjk} :] cette collection pour la prise en charge du chinois,
  du japonais et du coréen a été scindée en collections de langues individuelles
...
\item[\MP{} :] nouveau système de nombres \code{binary}; nouvelle activation
  ...
  à \prog{up*tex}.
\item[Mac\TeX :] mises à jour du paquet Ghostscript inclus pour la prise en
  charge CJK.  Le panneau de préférence de la distribution \TeX\ fonctionne
  maintenant sur Yosemite (\macOS~10.10). Les valises de polices du type
  « ressources » (« \emph{resource-fork font suitcases} »), qui ont des noms
  généralement sans extension, ne sont plus prises en charge par \XeTeX; les
  valises de polices du type « data » (extension \code{.dfont}) continuent elles
  à l'être.
\item[Infrastructure :] le script \prog{fmtutil} a été réimplémenté pour lire
...
\item[Plateformes :] \pkgname{*-kfreebsd} supprimé, puisque \TL{} est maintenant
  facilement disponible au travers des mécanismes de plateformes systèmes. Prise
  en charge pour quelques plateformes additionnelles disponibles en tant que
  binaires personnalisés (\url{https://tug.org/texlive/custom-bin.html}).  De
plus, quelques plateformes sont désormais omises du \DVD{} (simplement pour
...
\item[pdf\TeX :] trois nouvelles primitives \cs{pdfinfoomitdate},
  \cs{pdfsuppressptexinfo}, et \cs{pdftrailerid}, pour contrôler les valeurs
  apparaissant dans la sortie qui changent normalement à chaque exécution. Ces
  fonctionnalités ne concernent que les sorties PDF, et non les sorties DVI.
...
\item[Autres utilitaires :]\
  ...
  \item \code{upmendex} est un programme de création d'index, presque
    complètement compatible avec \code{makeindex}, proposant une prise en charge
    pour le classement Unicode, entre autres différences.
...
\item[Xe\TeX :] mathématiques Unicode/OpenType maintenant basées sur le prise en
  charge de la table MATH HarfBuzz's ; quelques corrections de bogues.
\item[Dvips :] le \enquote{special} de taille de papier pris en compte est le
  ...
  compte.
\item[ep\TeX, eup\TeX :] nouvelles primitives provenant de pdf\TeX :
  \begin{itemize}
...
\item[Sync\TeX :] l'extension du nom du fichier temporaire est désormais
  \code{.synctex(busy)}, et non plus \code{.synctex.gz(busy)} (absence
  ...
  conséquence.
\item[Autres utilitaires :] \code{texosquery-jre8} est un programme
  multi-plateforme nouveau permettant de retrouver la \enquote{locale} et autres
  ...
  restreintes. (Les anciennes versions de JRE sont prises en charge par
  texosquery, mais ne peuvent être activées en mode restreint car elles ne sont
  plus prises en charge par Oracle, même pour des problèmes de sécurité.)
...
\item[Xe\TeX :] la fonction \code{/Rotate} est désormais utilisable lors de
  ...
  primitives.
\item[Mac\TeX :] voir les changements de prise en charge de version ci-dessous.
  De surcroît, et pour plus de limpidité, les fichiers installés par Mac\TeX\
  dans le dossier \code{/Applications/TeX/} ont subi une réorganisation ; ce
  dossier contient désormais, au premier niveau, quatre applications graphiques
...
\item[Xe\TeX :]\leavevmode{}
  \begin{itemize}
  \item nouvelles primitives :
    \begin{itemize}
    \item \cs{expanded} ;
    \item \cs{creationdate} ;
    \item \cs{elapsedtime} ;
    \item \cs{filedump} ;
    \item \cs{filemoddate} ;
    \item \cs{filesize} ;
    \item \cs{resettimer} ;
    \item \cs{normaldeviate} ;
    \item \cs{uniformdeviate} ;
    \item \cs{randomseed} ;
    \end{itemize}
  \item \cs{Ucharcat} étendu pour produire des caractères actifs.
  \end{itemize}
\item[\code{tlmgr} :] prise en charge de \code{curl} comme programme de
  téléchargement ; si disponibles, utilise \code{lz4} et gzip avant \code{xz}
  pour les sauvegardes locales ; en ce qui concerne les programmes de
  compression et de téléchargement, priorité aux exécutables fournis par le
  système d'exploitation sur ceux fournis par la \TL{}, à moins que la variable
  d'environnement \code{TEXLIVE\_PREFER\_OWN} soit configurée.
...
\item[Utilitaires :] \
  ...
  l'implémentation CWEB dans la \TL{}, avec une prise en charge élargie des
  dialectes, et incluant le programme \code{ctwill} permettant de créer des
  mini-index.
  ...
  \item[\code{dvispc} :] crée un fichier \dvi{} indépendant des pages vis-à-vis
    des « specials ».
  \end{description}
\item[Mac\TeX{} :] \code{x86\_64-darwin} prend en charge désormais les systèmes
  10.12 et plus (Sierra, High Sierra, Mojave) ; \code{x86\_64-darwinlegacy}
  prend en charge encore les systèmes 10.6 et plus. Le correcteur orthographique
  Excalibur n'est plus inclus puisqu'il requiert la prise en charge des
...
\item[Général :]\
  ...
  \item Nouvelle option \texttt{--cnf-line} pour \texttt{kpsewhich},
    \texttt{tex}, \texttt{mf}, et tous les autres moteurs, pour prendre en
    charge une configuration arbitraire en ligne de commande.
  ...
  \end{itemize}
\item[ep\TeX, eup\TeX :] plusieurs nouvelles primitives, principalement pour la
  prise en charge inter-moteurs : \cs{Uchar}, \cs{Ucharcat},
  \cs{current(x)spacingmode}, \cs{ifincsname} ; également, révision de
  \cs{fontchar??}\ et \cs{iffontchar}. Pour eup\TeX\ uniquement :
  \cs{currentcjktoken}.
...
\item[Xe\TeX :] corrections pour \cs{Umathchardef}, \cs{XeTeXinterchartoks},
  \cs{pdfsavepos}.
\item[Dvips :] codages de sortie pour les polices bitmap, pour de meilleures
...
\item[Mac\TeX :] Mac\TeX\ et \texttt{x86\_64-darwin} requièrent désormais la
  ...
  \texttt{x86\_64-darwinlegacy} prend en charge 10.6 et plus récent. Mac\TeX\
  est certifié et les programmes en ligne de commande ont des
  \enquote{lancements renforcés} (\emph{hardened runtimes}), comme maintenant
  requis par Apple pour l'installation des paquets. BibDesk et \TL{} Utility ne
  sont pas dans Mac\TeX\ parce qu'ils ne sont pas certifiés, mais un fichier
  \filename{README} donne la liste des URL où ils peuvent être obtenus.
\item[\texttt{x86\_64-darwinlegacy} :] abandon de la prise en charge de X11,
  y compris \cmdname{xdvi}.
\item[\code{tlmgr} et infrastructure :]\
  ...
  \item Utilisation des noms de fichiers versionnés pour les conteneurs de paquets,
    comme dans : \texttt{tlnet/archive/\textsl{pkgname}.rNNN.tar.xz}. Cela
    devrait être invisible pour les utilisateurs, mais c'est un changement
    notable dans la distribution.
...
\item[Général :]\
  ...
  \item Les modifications apportées par Donald Knuth pour la mise au point 2021
    ...
    également disponibles sur le CTAN sous les packages \code{knuth-dist} et
  ...
  \item (Sauf dans l'original \TeX{}.)  Nouveau paramètre entier
    \cs{tracingstacklevels} qui, si positif ainsi que \cs{tracingmacros},
    provoque un préfixe indiquant la profondeur de la macro-expansion à sortir
    sur chaque ligne de journal pertinente (par exemple, |~..| à la profondeur
    2).  En outre, l'enregistrement des macros est tronqué à une profondeur
    $\geqslant$ la valeur du paramètre.

  \end{itemize}

\item[Aleph :] Le format \LaTeX\ basé sur Aleph, appelé \code{lamed}, a été
  supprimé. Le binaire \code{aleph} lui-même est toujours inclus et pris en
  charge.
...
\item[MetaPost :]\
  \begin{itemize}
  \item |SOURCE_DATE_EPOCH| : prise en charge de variable d'environnement pour
  ...
  \item Valeur de \texttt{epsilon} modifiée en modes binaire et décimal, de
    sorte que la fonction |mp_solve_rising_cubic| fonctionne comme prévu.
  \end{itemize}
...
\item[pdf\TeX{} :]\
  ...
  \item Dump des assignations \cs{pdfglyphtounicode} dans le fichier
    \texttt{fmt}.
  \item Source : prise en charge de \texttt{poppler} supprimé car il était trop
...
\item[Dvipdfmx :]\
  ...
  \item Ghostscript est maintenant par défaut invoqué en toute sécurité ; pour
    outrepasser cela (supposant ainsi que tous les fichiers d'entrée sont
    fiables), utilisez \verb|-i dvipdfmx-unsafe.cfg|.
    ...
    {\small \verb|xetex -output-driver="xdvipdfmx -i dvipdfmx-unsafe.cfg -q -E" foo|}

   \item Si un fichier image n'est pas trouvé, interruption avec un statut
    \enquote{mauvais}.
  \item Syntaxe spéciale étendue pour la prise en charge des couleurs.
  ...
  \item Prise en charge expérimentale pour les |fnt_def| étendus de
    \code{dviluatex}.
...
\item[Dvips :]\
  ...
  \item Prise en charge de la nouvelle fonctionnalité de la police virtuelle
    comme solution de repli sur la définition de la police japonaise.
  \end{itemize}

\item[Mac\TeX{} :] Mac\TeX{} et son nouveau dossier binaire
  \texttt{universal-darwin} nécessitent maintenant \macOS{} 10.14 ou plus
  (Mojave, Catalina, et Big~Sur) ; le dossier binaire |x86_64-darwin| n'est plus
  présent. Le dossier binaire |x86_64-darwinlegacy|, disponible uniquement avec
  l'installateur Unix \texttt{install-tl}, prend en charge les versions 10.6 et
  plus récentes.
...
\item[Général :]\leavevmode
  ...
  \item \code{tangle}, \code{weave} : prise en charge d'un troisième argument
    facultatif pour spécifier le fichier de sortie.
...
\item[ep\TeX{}, eup\TeX{} :]\leavevmode
  ...
  \item Nouvelles primitives :
    \begin{itemize}
    \item \cs{lastnodefont} ;
    \item \cs{suppresslongerror} ;
    \item \cs{suppressoutererror} ;
    \item \cs{suppressmathparerror}.
    \end{itemize}
...
\item[p\TeX{}\ et al. :]\leavevmode
  \begin{itemize*}
  \item Mise à jour majeure de p\TeX{} vers la version 4.0.0 pour mieux prendre
    en charge la version actuelle de \LaTeX{}.
...
\item[Xe\TeX{} :] \texttt{xetex-unsafe} et \texttt{xelatex-unsafe}, nouveaux
  scripts enveloppants (« wrappers ») pour une invocation plus simple des
  documents nécessitant à la fois \XeTeX{} et les opérateurs de transparence
  PSTricks, ce qui est intrinsèquement dangereux (jusqu'à ce que la
...
\item[Dvips :] Par défaut, pas de tentative d'ajustement automatique de la prise
  en charge pour les formats de papier pivotés ; la nouvelle option
...
\item[Mac\TeX{} :] Mac\TeX{} et son dossier binaire \texttt{universal-darwin}
  nécessitent \macOS{} 10.14 ou supérieur (Mojave, Catalina, Big~Sur,
  Monterey). Le dossier binaire |x86_64-darwinlegacy|, disponible uniquement
  avec le \texttt{install-tl} d'Unix, prend en charge les versions 10.6
  (Snow~Leopard) et plus récentes.
...
\item[Windows :] Comme annoncé précédemment, \TL{} contient maintenant des
  ...
  de travail pour les utilisateurs de Windows, mais il n'y avait pas de
  meilleure solution. Si vous avez besoin de la prise en charge de Windows
  32~bits, vous devrez installer \TL{} 2022.  À partir de Windows~11, Windows
  sur ARM prend en charge l'émulation des AMD/Intel 64-bit et peut donc utiliser
  la \TL{} actuelle. Cependant, Windows~10 sur ARM ne peut pas émuler Intel/AMD
  64-bit, et aura donc besoin d'un TeX Live 32-bit. Voir la page web \TL{}
...
\item[Extensions inter-moteurs\footnote{Sauf dans les versions originales \TeX{}
  ...
  moment \cs{shipout}, comme avec un non-\verb|\immediate\write|.
...
\item[Lua\TeX :]\leavevmode{}
  ...
  \item dans Lua\TeX\ 1.17.0, publié en mai 2023 : la bibliothèque socket est
    maintenant désactivée par défaut ; une nouvelle option de ligne de commande
    \code{--socket} l'active, ainsi que \code{--shell-escape} (et non
    \code{--shell-restricted}. De plus, la bibliothèque \verb|mime| est maintenant
    toujours disponible, et les nouvelles fonctions \code{os.socketsleep} et
    \code{os.socketgettime} sont également toujours disponibles. Enfin,
    certaines façons obscures de contourner diverses fonctionnalités de sécurité
    ont été corrigées.
  \end{itemize*}
...
\item[Xe\TeX :] correction :
  \begin{itemize}
  \item du calcul de \cs{topskip} et \cs{splittopskip} lorsque
    \cs{XeTeXupwardsmode} est actif ;
  \item de « \cs{special} retardé » décrit ci-dessus.
  \end{itemize}
...
\item[\BibTeX{}u :]\leavevmode{}
  ...
  \item Cette variante de Bib\TeX{} est principalement compatible avec \BibTeX,
    avec une bien meilleure prise en charge multilingue (basé sur Unicode). Elle
    est présente dans \TL{} depuis quelques années.
...
\item[Mac\TeX{} :] \leavevmode{}
  ...
  \item Mac\TeX{} et son dossier binaire \texttt{universal-darwin} nécessitent
    \macOS{} 10.14 ou supérieur (Mojave, Catalina, Big~Sur, Monterey,
    Ventura). Le dossier binaire \texttt{x86\_64-darwinlegacy}, disponible
    uniquement avec le programme Unix \texttt{install-tl}, prend en charge les
    versions 10.6 (Snow~Leopard) et ultérieures.
...
\item[Plates-formes :] \leavevmode{}
  ...
  \item Le répertoire binaire \texttt{i386-cygwin} a disparu, car Cygwin ne ne
    prend plus en charge i386.
 \end{itemize*}
\end{description}

\htmlanchor{news}
\subsection{Présent : 2024}
\label{sec:tlcurrent}

\begin{description}
\item[Changements inter-moteurs (sauf \TeX{} d'origine, e-\TeX{} et Lua\TeX) :]
  \leavevmode{}
  \begin{itemize}
  \item Éviter les plantages lors de l'utilisation détournée des accolades
    \cs{output} et \cs{write}.
  \item Petite optimisation de la boucle interne.
  \end{itemize}
  (Ces deux changements sont dus à David Fuchs.)
\item[Lua\TeX :] \leavevmode{}
  \begin{itemize*}
  \item Les vérifications au niveau de Lua pour l'écriture dans les fichiers
    sont maintenant similaires aux vérifications au niveau de \TeX.
  \item Si la valeur Lua \code{texconfig.trace\_extra\_newline} est définie, des
    lignes vierges sont affichées dans le journal entre les paramètres de la
    macro. En 2020--2023, c'était le cas par défaut ; avant cela, de telles
    lignes vides n'étaient jamais affichées, ce qui est le comportement des
    autres moteurs.
  \item Par le passé, \code{texlua} sous Windows (pas \code{luatex}, ni aucune
    autre plateforme), effectuait en interne l'expansion des caractères
    génériques. Ce n'est plus le cas aujourd'hui.
\end{itemize*}
\item[MetaPost :] La bibliothèque MPFI est maintenant incluse pour
  l'arithmétique d'intervalle ; activée avec \texttt{--numbersystem=interval}.
\item[pdf\TeX :] \leavevmode{}
  \begin{itemize*}
  \item Extraction de texte plus fiable sur les systèmes BigEndian.
  \item Davantage de vérifications concernant les fichiers pfb, fmt, pdf
    corrompus.
  \item Écriture d'un pdf corrompu évitée en cas d'inclusions pdf
    inhabituelles.
  \item Manuel converti en \LaTeX{} et révisé en profondeur, bien que les mises
    à jour substantielles soient minimales.
  \end{itemize*}
\item[ep\TeX{}, eup\TeX{} :] \leavevmode{}
  \begin{itemize*}
  \item Nouvel utilitaire de conversion de codage simple \texttt{ptekf}.
  \item Construction de \texttt{eptex} désormais plus séparée ; il fonctionne dans le
    mode d'encodage hérité de \texttt{euptex}.
  \end{itemize*}
\item[Xe\TeX :] Les caractères Unicode autres que BMP dans les noms de fichiers
  sont pris en charge.
\item[Dvipdfmx :] \leavevmode{}
  \begin{itemize*}
  \item Calcul du décalage de fuseau horaire identique à celui de pdf\TeX.
  \item Gestion des sélecteurs de variation pour tenir compte de la nouvelle
    CMap Adobe-Japan1-UCS2.
  \item Solution de repli des caractères multi-octets dans les polices
    virtuelles vers les polices OFM pour (u)p\TeX.
  \item Prise en charge du mot-clé \code{named} dans le spécial
    \code{pdf:image}.
  \end{itemize*}
\item[Kpathsea :] \leavevmode{}
  \begin{itemize*}
  \item Prise en charge d'une vérification étendue des noms de fichiers sûrs qui
    permet également d'écrire sous \code{TEXMF}[\code{SYS}]\code{VAR}, pour
    Lua\AllTeX{} ; nouvelles fonctions et options \code{kpsewhich}
    correspondantes.
  \item Prise en charge d'une nouvelle variable \code{TEXMF\_OUTPUT\_DIRECTORY}
    (à côté de la traditionnelle \code{TEXMFOUTPUT}), de sorte que les
    sous-programmes puissent avoir accès à un paramètre
    \texttt{--output-directory} lors d'une invocation du moteur.
  \end{itemize*}
\item[CWEB :] \leavevmode{}
  \begin{itemize*}
  \item Prise en charge de Lua\TeX{} en tant que moteur \TeX{} dorsal
    (\emph{back-end}).
  \item Correction d'un dépassement de tampon sur les lignes sans point d'arrêt.
  \end{itemize*}
\item[\code{tlmgr} :] \leavevmode{}
  \begin{itemize*}
  \item Nouvelle action \code{bug} pour aider à trouver des informations sur les
    rapports de bogues.
  \item Utilisation de notre propre paquet de certificats racine sur toutes les
    plateformes, pour prendre en charge LWP plus largement, notamment sur macOS.
  \item Détection d'un dépôt distant plus ancien que l'installation locale
    (c'est-à-dire un miroir périmé), et alors prévention de la mise à jour, afin
    d'éviter les suppressions incorrectes de paquets.
  \item Prise en charge du paramètre de taille de papier \ConTeXt{}.
  \item Élagage des sauvegardes des fichiers \texttt{tlpdb} pour les dépôts
    miroirs (\texttt{main} et \texttt{tlcontrib}).
  \end{itemize*}
\item[Plateformes :] \leavevmode{}
  \begin{itemize*}
  \item L'image \ISO\ comprend à nouveau des binaires pour toutes les
    plateformes. Ces dernières années, certains ensembles binaires ont été
    élagués, mais cette année, nous ne créons pas de \DVD{} physique au profit
    du groupe d'utilisateurs, alors autant tout inclure à nouveau. En outre,
    même une image élaguée au maximum est trop volumineuse pour tenir sur un
    \DVD{} simple couche, de sorte que l'élagage ne présente aucun avantage pour
    les graveurs volontaires de 2024. Plus d'informations sur
    \url{https://tug.org/texcollection/}.
  \item Robert Alessi fournit des binaires pour une installation native de \TL{}
    sur OpenBSD à \url{https://sr.ht/~ralessi/texlive-openbsd/}.
\end{itemize*}
\item[Mac\TeX :] contient maintenant la dernière version de Ghostscript 10.02.1,
  les mises à jour des quatre programmes d'interface graphique, ainsi que (comme
  toujours) tous les nouveaux changements de \TL. Les binaires sont toujours
  compatibles avec macOS~10.14 (Mojave, 2018) et les versions
  ultérieures. (Comme d'habitude, les versions antérieures de Mac sont prises en
  charge par les binaires \texttt{x86\_64-darwinlegacy}, qui font partie de \TL,
  mais pas Mac\TeX{}).
\end{description}
...
\begin{quote}
  \email{tex-live@tug.org} {\small (il n'est pas nécessaire de s'abonner pour pouvoir poster)}\\
