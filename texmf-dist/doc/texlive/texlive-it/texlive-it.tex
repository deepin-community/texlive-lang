...
% arara: pdflatex
% arara: showfile
% $Id: texlive-it.tex 58653 2021-03-23 17:25:09Z karl $
...
\item Per Unix, lo script di installazione è \filename{install-tl}; su
  Windows, bisogna invece lanciare \filename{install-tl-windows}.
  L'installatore opererà in modalità grafica dando l'opzione
  \code{-gui} (predefinita per Windows e \MacOSX), o in modalità testuale
...
invocato per molti formati comuni, come \prog{etex}, \prog{latex},
...
di \TeX. Quando è invocato come \filename{luatex}, si comporta come un
interprete Lua autonomo. Il suo sito web è \url{http://www.luatex.org/}
e il manuale di riferimento è \OnCD{texmf-dist/doc/luatex/base/luatexref.pdf}.
...
disponibili, ma ora fanno le stesse cose di \code{-gui}. Per un elenco
...
Le opzioni \GUI{} \texttt{perltk}/\texttt{expert} e \texttt{wizard} ora
chiamano il normale installatore grafico.
...
\dirname{/usr/local/texlive/2021} sotto
Unix e |%SystemDrive%\texlive\2021| sotto Windows. Questa configurazione
...
\cmdname{evince} e \cmdname{okular} sono comunemente usati. Per Windows,
...
\end{alltt}
Dovete avere X in esecuzione affinché \cmdname{xdvi} funzioni;
se non è così, oppure se la vostra variabile d'ambiente
\envname{DISPLAY} non è impostata correttamente, otterrete un errore
...
\item[Editor orientati a \TeX] C'è un'ampia scelta ed è una questione di
  ...
  \item \cmdname{TeXstudio} è nato come un fork di \cmdname{Texmaker}
        e sono state poi aggiunge altre funzionalità; disponibile su
        \url{https://texstudio.org/} e nella distribuzione pro\TeX{}t.
...
i valori predefiniti di \envname{TEXMFHOME}, \envname{TEXMFVAR} e
\envname{TEXMFCONFIG} pari rispettivamente a \envname{TEXMFLOCAL},
\envname{TEXMFSYSVAR} e \envname{TEXMFSYSCONFIG}; di conseguenza la
...
\file{.../2021/texmf.cnf}. Se dovete apportare delle modifiche ai valori
predefiniti (di norma non è necessario), questo è il posto in cui
piazzarle. Il file di configurazione principale è
\file{.../2021/texmf-dist/web2c/texmf.cnf}. Non dovreste modificare
...
\item[\texttt{hash\_extra}]
  ...
  controllo; il suo valore predefinito è \texttt{600000}.
\end{ttdescription}
...
directory non esistente per questo.
...
dalle sue distribuzioni per il \TeX{} giapponese W32TEX e W64TEX
...
\item Hironori Kitagawa, per la manutenzione di (e)p\TeX\ e il relativo
supporto.
...
usata in \TL{}, e per continuare la manutenzione del CWEB originale.

\item Takuji Tanaka per la manutenzione di (e)(u)p\TeX\ e il relativo
supporto.
...
Generale: \begin{itemize}
\item Sono incorporati i cambiamenti di Donald Knuth per la sua sistematina
del 2021 a \TeX\ e Metafont
(\url{https://tug.org/TUGboat/tb42-1/tb130knuth-tuneup21.pdf}). Sono anche
disponibili su CTAN sotto forma dei pacchetti \code{knuth-dist} e
\code{knuth-local}. Come atteso, i fix sono per casi oscuri e non riguardano
nessun comportamento pratico.

\item Tranne che nel \TeX\ originale: se \cs{tracinglostchars} è impostato a
3 o più, i caratteri mancati daranno origine ad un errore, non semplicemente
ad un messaggio nel file di log e il codice del carattere mancante sarà
mostrato in esadecimale.

\item Tranne che nel \TeX\ originale: un nuovo parametro intero
\cs{tracingstacklevels} che se positivo, e \cs{tracingmacros} è anch'esso
positivo, provoca l'aggiunta di un prefisso che indica la profondità di
espansione della macro all'output nella relativa riga di log (per esempio,
|~..| a profondità 2). Inoltre il log delle macro è troncato alla profondità
$\ge$ del valore del parametro

\end{itemize}

Aleph: il formato \LaTeX\ basato su Aleph, chiamato \code{lamed}, è stato
rimosso. L'eseguibile \code{aleph} stesso è ancora incluso e supportato.

Lua\TeX: \begin{itemize*}
\item Lua 5.3.6.
\item Callback per il livello di annidamento usata in \cs{tracingmacros},
come variante generalizzata del nuovo \cs{tracingstacklevels}.
\item Glifi matematici marchiati come protetti per evitare l'elaborazione
come testo.
\item Rimossa la compensazione larghezza/ic per il codice matematico
tradizionale.
\end{itemize*}

MetaPost: \begin{itemize*}
\item Suppporto alla variabile d'ambiente |SOURCE_DATE_EPOCH| per output
riproducibile.
\item Evitato il \texttt{\%} finale errato in \texttt{mpto}.
\item Documentata l'opzione \texttt{-T}, altre correzioni al manuale.
\item Valore di \texttt{epsilon} cambiato in modalità binaria e decimale,
così che |mp_solve_rising_cubic| funzioni come atteso.
\end{itemize*}

pdf\TeX{}: \begin{itemize*}
\item Nuove primitive \cs{pdfrunninglinkoff} e
\cs{pdfrunninglinkon}; ad esempio, per disattivare la generazione dei link
in intestazioni e pié di pagina.
\item Avverte invece di uscire con l'errore ``\cs{pdfendlink} ended up in
different nesting level than \cs{pdfstartlink}''.
\item Stampa le assegnazioni di \cs{pdfglyphtounicode} nel file \texttt{fmt}.
\item Sorgenti: supporto a \texttt{poppler} rimosso in quanto troppo
difficile mantenerlo sincronizzato con la linea di sviluppo. In TL nativa,
pdf\TeX\ ha sempre usato \texttt{libs/xpdf}, che è un codice ridotto e
adattato da \texttt{xpdf}.
\end{itemize*}

Xe\TeX{}: correzioni per il kerning matematico.

Dvipdfmx: \begin{itemize*}
\item Ghostscript è ora invocato in modalità sicura per default; per annullare
    (assumendo che tutti i file di input siano fidati), usate
    \verb|-i dvipdfmx-unsafe.cfg|. Questo è richiesto per usare PSTricks con
    \XeTeX, come in:
    \verb|xetex -output-driver="xdvipdfmx -i dvipdfmx-unsafe.cfg -q -E" ...|
\item Se il file di un'immagine non è trovato, esce con stato di errore.
\item Sintassi special estesa per il supporto dei colori.
\item Special per la manipolazione di |ExtGState|.
\item Special di compatibilità \code{pdfcolorstack} e \code{pdffontattr}.
\item Supporto sperimentale per |fnt_def| esteso di \code{dviluatex}.
\item Supporto per nuova funzionlità dei font virtuali per ripiego nella
definizione di font giapponesi.
\end{itemize*}

Dvips: \begin{itemize*}
\item Il titolo di default del documento PostScript è ora il nome base del
file di input e può essere sostituito tramite la nuova opzione \texttt{-title}.
\item Se un \texttt{.eps} o un altro file di immagine non è trovato, esce
con stato di errore.
\item Supporto per nuova funzionlità dei font virtuali per ripiego nella
definizione di font giapponesi.
\end{itemize*}

Mac\TeX{}: Mac\TeX{} e la sua nuova directory degli eseguibili
\texttt{universal-darwin} ora richiede macOS 10.14 o successivi (Mojave, Catalina
e Big~Sur); la directory degli eseguibili |x86_64-darwin| non è più presente.
La directory |x86_64-darwinlegacy|, disponibile solo nel \texttt{install-tl} per
Unix, supporta le versioni 10.6 e successive.

Questo è un anno importante per il Macintosh perché Apple ha introdotto a
novembre le macchine con processore ARM e venderà e supporterà sia macchine ARM
che Intel per svariati anni. Tutti i programmi in \texttt{universal-darwin} hano
codice eseguibile sia per ARM che per Intel. Entrambi i binari sono compilati
dallo stesso sorgente.

I programmi aggiuntivi Ghostscript, LateXiT, \TeX{} Live Utility e TeXShop sono
tutti universali e sono firmati con un runtime indurito, quindi sono tutti
inclusi in Mac\TeX{} quest'anno.

\code{tlmgr} e infrastruttura: \begin{itemize*}
\item mantenuto un solo backup del \texttt{texlive.tlpdb} del repository principale.
\item maggiore portabilità tra sistemi e versioni di Perl.
\item \texttt{tlmgr -info} mostra i nuovi campi \texttt{lcat-*} e \texttt{rcat-*}
per i dati del Catalogo locali vs. remoti.
\item il log completo dei sottocomandi è stato spostato nel nuovo file di log
\texttt{texmf-var/web2c/tllmgr-commands.log}.
\end{itemize*}


\subsection{Futuro}

Abbiamo intenzione di continuare a rilasciare nuove versioni di \TL\ e vorremmo
fornire più documentazione, più programmi e un insieme sempre migliore e meglio
controllato di font, di macro e di tutto quanto legato a \TeX. Questo lavoro è
fatto completamente da volontari nel loro tempo libero e quindi c'è sempre
molto da fare. Visitate il sito \url{https://tug.org/texlive/contribute.html}.
